...
function BrowserExtensionButton() {
  const [browserInfo, setBrowserInfo] = useState({
    name: "Chrome",
    icon: <Chrome className="size-10 group-hover:rotate-12 transition-transform inline-block" />,
    url: "https://github.com/KitsuneKode/yt-playlist-dedupe/releases/latest/download/yt-dedupe-chrome.zip",
  });
  ...
  useEffect(() => {
    ...
    if (userAgent.includes("firefox")) {
      setBrowserInfo({
        name: "Firefox",
        icon: (
          <span className="text-4xl group-hover:rotate-12 transition-transform inline-block">
            🦊
          </span>
        ),
        url: "https://github.com/KitsuneKode/yt-playlist-dedupe/releases/latest/download/yt-dedupe-firefox.zip",
      });
    } else {
      setBrowserInfo({
        name: "Chrome",
