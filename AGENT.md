...
- Built CLI output in `dist/` via `tsconfig.build.json`
...
- [tsconfig.build.json](/home/kitsunekode/Projects/cli-tools/yt-playlist-dedupe/tsconfig.build.json): build config for publishable output
...
- `bun run smoke:built`: verify built CLI runs with Node
...
9. Confirm npm auth with `npm whoami`.
10. Publish with `npm publish --access public` or `bun run release`.
...
- Keep package output Node-compatible because the published bin points to `dist/index.js`
